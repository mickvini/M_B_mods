...
function UpgradeUnit(oUnitToUpgrade, bUpdateUpgradeTracker, iOptionalWait)
    ...
    --M&B: dont upgrade factories until at least 6 extractors have been upgraded. Factory upgrades are a mass luxury; rushing them before raising mass income (mex upgrades) stalls mass, then the higher-tier factory maintenance stalls energy, the bot spams pgens and spirals. Mex-first. Only refCategoryFactory is gated - mex and ACU upgrades are unaffected. (Only ever runs for the AI - UpgradeUnit is M28-only.)
    if M28Utilities.IsMBModActive() and EntityCategoryContains(M28UnitInfo.refCategoryFactory, oUnitToUpgrade.UnitId) then
        ...
        local iMNBUpgradedMex = 0
        if oMNBFacBrain then
            iMNBUpgradedMex = (oMNBFacBrain:GetCurrentUnits(M28UnitInfo.refCategoryMex * categories.TECH2) or 0) + (oMNBFacBrain:GetCurrentUnits(M28UnitInfo.refCategoryMex * categories.TECH3) or 0)
        end
        if iMNBUpgradedMex < 6 then
            if bDebugMessages == true then LOG(sFunctionRef..': M&B blocking factory upgrade - only '..iMNBUpgradedMex..' upgraded extractors (need 6)') end
            M28Profiler.FunctionProfiler(sFunctionRef, M28Profiler.refProfilerEnd)
            return
        end
    end
