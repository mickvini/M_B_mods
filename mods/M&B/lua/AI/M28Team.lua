    ...
    if M28Map.bMapLandSetupComplete and M28Map.bWaterZoneInitialCreation then
        ...
        for iBrain, oBrain in tTeamData[iM28Team][subreftoFriendlyActiveM28Brains] do
            if bIncludeHumanBrains or oBrain.BrainType == 'AI' then
                --M&B: M28's tracked base-income fields (refiGrossEnergyBaseIncome etc.) DONT see M&B's runtime boosts (generator boost patch #19 via SetProductionPerSecondEnergy; ACU mass boost patch #20), so they under-report gross energy by ~17x (5130 tracked vs 85980 real). That made HaveLowPower perpetually true (grossEnergy<80000 was ALWAYS true) -> the bot spammed generators forever (+53900 surplus) and never built fabs. For M&B use the real FA brain API (the author noted this gap in the old commented-out block just below but left it disabled due to a 1-tick vanilla delay; in M&B the boost gap dwarfs that delay). Vanilla mods keep the tracked fields untouched.
                if M28Utilities.IsMBModActive() then
                    local iMBREner = oBrain:GetEconomyIncome('ENERGY') or 0
                    local iMBRMass = oBrain:GetEconomyIncome('MASS') or 0
                    tTeamData[iM28Team][subrefiTeamGrossEnergy] = tTeamData[iM28Team][subrefiTeamGrossEnergy] + iMBREner
                    tTeamData[iM28Team][subrefiTeamGrossMass] = tTeamData[iM28Team][subrefiTeamGrossMass] + iMBRMass
                    tTeamData[iM28Team][subrefiTeamNetEnergy] = tTeamData[iM28Team][subrefiTeamNetEnergy] + (oBrain:GetEconomyTrend('ENERGY') or 0)
                    tTeamData[iM28Team][subrefiTeamNetMass] = tTeamData[iM28Team][subrefiTeamNetMass] + (oBrain:GetEconomyTrend('MASS') or 0)
                else
