    ...
    if tNearestPlateauOrZeroAndZoneSegmentOverride[iSegmentX][iSegmentZ] then
        ...
        if not(iPlateau) or (not(tLZData) and tWaterZoneBySegment[iSegmentX][iSegmentZ] == nil) then
            if (iSegmentX <= -30 or iSegmentZ <= -30 or iSegmentX >= iMaxLandSegmentX + 30 or iSegmentZ >= iMaxLandSegmentZ + 30) then
                --E.g. RNG can sometimes send air units far outside the map area. M&B: this is a known benign/expected case (not an error), so only log with debug on — was spamming the playtest log as an M28Warning.
                if bDebugMessages == true then LOG(sFunctionRef..': Position far outside playable area ('..(iSegmentX or '?')..','..(iSegmentZ or '?')..'), skipping zone search (benign - air units can be off-map)') end
            else
...
function SetupPlayableAreaAndSegmentSizes(rCampaignPlayableAreaOverride)
    ...
    if not(bMapLandSetupComplete) then --e.g. if this is a campaign we may want to change playable area size
        ...
        import('/mods/M&B/lua/AI/M28Engineer.lua').iLandZoneSegmentSize = iLandZoneSegmentSize
            ...
            if oFabBrain and oFabBrain.GetListOfUnits and not(oFabBrain:IsDefeated()) then
                ...
                for _, oFab in tMNBFabs do
                    if oFab and oFab.GetPosition then
                        local tFP = oFab:GetPosition()
                        local iFP, iFLZ = GetPlateauAndLandZoneReferenceFromPosition(tFP, false, oFab)
                        if iFP == iPlateau and iFLZ == iLandZone then --only fabs in THIS zone
                            for _, tOff in tiFabOffset do
                                local tFP2 = {tFP[1] + tOff[1], 0, tFP[3] + tOff[2]}
                                tFP2[2] = GetSurfaceHeight(tFP2[1], tFP2[3])
                                if M28Conditions.CanBuildStorageAtLocation(tFP2) then
                                    table.insert(tLZData[subrefLZOrWZMassStorageLocationsAvailable], tFP2)
                                end
                            end
                        end
                    end
...
function AddGameEnderTemplateInfoToTable(tMidpoint, iPreferredSize)
    ...
    if bDebugMessages == true then LOG(sFunctionRef..': Adding game ender template info to table for midpoint='..repru(tMidpoint)..'; iPreferredSize='..iPreferredSize..'; Can we build a preferred size unit here='..tostring(M28Overseer.GetFirstActiveBrain():CanBuildStructureAt(import('/mods/M&B/lua/AI/M28Engineer.lua').tsBlueprintsBySize[iPreferredSize], tMidpoint)))
