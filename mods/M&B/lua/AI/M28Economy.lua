...
local M28Factory = import('/mods/M&B/lua/AI/M28Factory.lua')
...
local M28Navy = import('/mods/M&B/lua/AI/M28Navy.lua')
...
function UpgradeUnit(oUnitToUpgrade, bUpdateUpgradeTracker, iOptionalWait)
    ...
    --M&B SWITCH (event-driven, not polled): dont run the upgrade check until the target tier is researched. brain.MNB_TechUnlocked[N] flips on when study 9N00 completes (defaultunits.lua OnStopBeingBuilt). Before that, bail instantly -> 0 wasted GetUnitUpgradeBlueprint calls / upgrade attempts every cycle.
    if M28Utilities.IsMBModActive() then
        local sMNBUpgradesTo = oUnitToUpgrade:GetBlueprint().General.UpgradesTo
        if sMNBUpgradesTo then
            local oMNBTargetBP = __blueprints[sMNBUpgradesTo]
            local oMNBBrain = oUnitToUpgrade:GetAIBrain()
            local tMNBUnlocked = (oMNBBrain and oMNBBrain.MNB_TechUnlocked) or {}
            if oMNBTargetBP and oMNBTargetBP.Categories then
                local tMNBCats = oMNBTargetBP.Categories
                if (table.find(tMNBCats,'TECH2') and not tMNBUnlocked[2]) or (table.find(tMNBCats,'TECH3') and not tMNBUnlocked[3]) or ((table.find(tMNBCats,'TECH4') or table.find(tMNBCats,'EXPERIMENTAL')) and not tMNBUnlocked[4] and not tMNBUnlocked[5]) then
    ...
    --M&B: dont upgrade factories until MOST extractors are upgraded (>=70% T2/T3, min 4). Originally a flat '6' (#12), but that let factories upgrade before the bulk of mexes finished -> mass pit (matches the #41 revision). Mex-first: mex upgrades raise the mass income the factory upgrade depends on. Only refCategoryFactory is gated - mex and ACU upgrades are unaffected. (Only ever runs for the AI - UpgradeUnit is M28-only.)
    if M28Utilities.IsMBModActive() and EntityCategoryContains(M28UnitInfo.refCategoryFactory, oUnitToUpgrade.UnitId) then
        local oMNBFacBrain = oUnitToUpgrade:GetAIBrain()
        local iMNBTotalMex = 0
        ...
        if iMNBTargetMex < iMNBMexNeeded then
            if bDebugMessages == true then LOG(sFunctionRef..': M&B blocking factory upgrade - only '..iMNBTargetMex..' target-tier extractors of '..iMNBMexNeeded..' needed (50% of '..iMNBTotalMex..')') end
            M28Profiler.FunctionProfiler(sFunctionRef, M28Profiler.refProfilerEnd)
            return
        end
    end
    ...
    if bContinue then
        ...
        --M&B: disable the CanBuild-bypass for upgrades. In M&B tech tiers are research-locked (TECH2 after study 9200, TECH3 after 9300, T4 after 9400), so CanBuild=false legitimately means the tier isnt researched yet; the bypass would let the AI upgrade mexes/factories to T2 before researching 9200. __blueprints['sab9101'] (M&B research lab) only exists in M&B, so this only triggers there.
        if not(sUpgradeID) and not(oUnitToUpgrade[refbTriedIgnoringCanBuildForUpgrade]) and not(__blueprints['sab9101']) then
        ...
        if sUpgradeID and M28UnitInfo.IsUnitValid(oUnitToUpgrade) and bUpdateUpgradeTracker then
            ...
            --M&B: IsUnitRestricted is blind to M&B's per-army research tier-locks (AddBuildRestriction), and 'no valid upgrade ID' is the EXPECTED state in M&B (tier not researched yet, or factory HQ/support category quirks). Patch elsewhere (UpgradeUnit bypass-disable) ensures no illegal upgrade actually happens, so in M&B never treat this as an error — it flooded warnings (Count=256) and lagged the game.
            if sExpectedUpgradeID and (M28UnitInfo.IsUnitRestricted(sExpectedUpgradeID, oUnitToUpgrade:GetAIBrain():GetArmyIndex()) or M28Utilities.IsMBModActive()) then
                --Restricted (campaign/lobby), or M&B tier not yet researched / upgrade unavailable — not an error
...
function UpdateHighestFactoryTechLevelForBuiltUnit(oUnitJustBuilt)
    ...
    if oUnitJustBuilt:GetFractionComplete() == 1 and EntityCategoryContains(M28UnitInfo.refCategoryFactory + M28UnitInfo.refCategoryQuantumGateway, oUnitJustBuilt.UnitId) then
        ...
        if EntityCategoryContains(M28UnitInfo.refCategoryAllHQFactories, oUnitJustBuilt.UnitId) and not EntityCategoryContains(M28UnitInfo.refCategoryResearchFactory, oUnitJustBuilt.UnitId) then --M&B: research centre is a FACTORY HQ but not a unit-production factory; exclude it or it hits the 'Unrecognised factory type' else-branch (sFactoryRef stays nil) and crashes math.max at line ~650.
...
function UpdateHighestFactoryTechLevelForDestroyedUnit(oUnitJustDestroyed)
    ...
    if EntityCategoryContains(M28UnitInfo.refCategoryFactory, oUnitJustDestroyed.UnitId) then
        ...
        if EntityCategoryContains(M28UnitInfo.refCategoryAllHQFactories, oUnitJustDestroyed.UnitId) and not EntityCategoryContains(M28UnitInfo.refCategoryResearchFactory, oUnitJustDestroyed.UnitId) then --M&B: research centre is a FACTORY HQ but not a unit-production factory; exclude it or it falls through to the 'Unrecognised factory type' else-branch below (~line 748) when a lab is destroyed. Mirrors the BuiltUnit fix at line ~639.
...
function MNBSweepMexUpgrades(aiBrain)
    --M&B: the moment a tech tier is researched, put EVERY extractor on upgrade in parallel. In M&B mass is the only real constraint and mexes self-upgrade, so the vanilla 'one at a time, throttle the pace, assist with engineers' pacing is wrong - we want all extractors upgrading at once and let the engine spread the mass cost across them. Called from the mod (defaultunits.lua) when a research tier completes, for AI brains only.
    if not M28Utilities.IsMBModActive() then return end
    local tMexes = aiBrain:GetListOfUnits(M28UnitInfo.refCategoryMex, false, true)
    for _, oMex in tMexes do
        if M28UnitInfo.IsUnitValid(oMex) and not(oMex:IsUnitState('Upgrading')) and not(oMex:IsUnitState('BeingUpgraded')) and M28Conditions.SafeToUpgradeUnit(oMex) then
            local sMNBUpg = oMex:GetBlueprint().General.UpgradesTo
            --Gate on CanBuild, NOT on the MNB_TechUnlocked flag. The flag is set one research stage EARLY (MNB_TechUnlocked[2] is set when study 9100 / RESEARCHLOCKEDTECH1 completes), but the T1->T2 mex restriction is only actually lifted when study 9200 completes. CanBuild is the engine's authoritative per-army check, so it only returns true once the tier is REALLY unlocked (9200 -> T1->T2, 9300 -> T2->T3). This stops the sweep firing prematurely on 9100.
            if sMNBUpg and sMNBUpg ~= '' and oMex:CanBuild(sMNBUpg) then
...
function MNBSweepRadarUpgrades(aiBrain)
    --M&B: the moment T2 unlocks, upgrade T1 radars to T2 in parallel. M28 has no ConsiderUpgradingT1Radar (it only builds a separate T2 radar nearby, or upgrades T2->T3 omni via a very narrow trigger), so a bot in M&B sits on a single T1 radar forever and goes blind. M&B radar blueprints have an UpgradesTo chain (e.g. ueb3101->ueb3201->ueb3104). Gate strictly on CanBuild so this only fires once the T2-radar restriction is really lifted by research; T2->T3 omni is left to M28's own logic. Called from the mod (defaultunits.lua) on any tier unlock, like MNBSweepMexUpgrades.
    if not M28Utilities.IsMBModActive() then return end
    local tRadars = aiBrain:GetListOfUnits(M28UnitInfo.refCategoryRadar, false, true)
    for _, oRadar in tRadars do
        if M28UnitInfo.IsUnitValid(oRadar) and not(oRadar:IsUnitState('Upgrading')) and not(oRadar:IsUnitState('BeingUpgraded')) and M28Conditions.SafeToUpgradeUnit(oRadar) then
            if M28UnitInfo.GetUnitTechLevel(oRadar) == 1 then
                local sMNBUpg = oRadar:GetBlueprint().General.UpgradesTo
                if sMNBUpg and sMNBUpg ~= '' and oRadar:CanBuild(sMNBUpg) then
                    UpgradeUnit(oRadar, true)
                end
            end
        end
    end
...
function ConsiderFutureMexUpgrade(oMex, iOverrideSecondsToWait)
    ...
    --M&B: in M&B mass is the whole economy and mexes self-upgrade (no engineer-assist), so the vanilla 'one at a time, throttle, assist, wait 15-20min' logic is wrong - upgrade the moment the next tier is REALLY unlocked. Gate on CanBuild (engine per-army check), NOT on the MNB_TechUnlocked flag, which is set one stage early (study 9100 only unlocks the SECOND HALF of T1, not T2; T1->T2 mex only actually unlocks at study 9200). If not yet unlocked (CanBuild false), fall through to the vanilla logic below (which has proper WaitSeconds re-polling). Never ForkThread from here without a WaitSeconds first - that fork-bombs and freezes the game.
    if M28Utilities.IsMBModActive() and iMexTechLevel and iMexTechLevel <= 2 then
        local sMNBUpg = oMex:GetBlueprint().General.UpgradesTo
        if sMNBUpg and sMNBUpg ~= '' and oMex:CanBuild(sMNBUpg) and M28Conditions.SafeToUpgradeUnit(oMex) then
            UpgradeUnit(oMex, true)
            M28Profiler.FunctionProfiler(sFunctionRef, M28Profiler.refProfilerEnd)
            return
        end
    ...
    if M28UnitInfo.IsUnitValid(oMex) then
        --M&B: (after the vanilla wait) re-check the fast-path. Existing mexes built BEFORE the tier was researched slept through the wait above and wake up here; without this theyd fall into the vanilla one-at-a-time logic. Gate on CanBuild (engine per-army check), not the MNB_TechUnlocked flag (which is set one stage early - 9100 only unlocks T1's second half, T1->T2 mex unlocks at 9200). If the next tier is now REALLY unlocked and the mex is safe, upgrade immediately and return so all extractors upgrade in parallel.
        if M28Utilities.IsMBModActive() and iMexTechLevel and iMexTechLevel <= 2 then
            local sMNBUpg = oMex:GetBlueprint().General.UpgradesTo
            if sMNBUpg and sMNBUpg ~= '' and oMex:CanBuild(sMNBUpg) and M28Conditions.SafeToUpgradeUnit(oMex) then
                UpgradeUnit(oMex, true)
