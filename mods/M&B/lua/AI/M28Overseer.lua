...
local M28Chat = import('/mods/M&B/lua/AI/M28Chat.lua')
...
function GameSettingWarningsChecksAndInitialChatMessages(aiBrain)
    ...
    if bIncompatible then
        if bDontPlayWithM27 then
            ...
            M28Chat.SendMessage(aiBrain, 'SendGameCompatibilityWarning', 'Detected'..sIncompatibleMessage .. ' (v'..import('/mods/M&B/mod_info.lua').version..') if you come across M28AI issues with these settings/mods let maudlin27 know via Discord', 0, 10)
        end
    end
    ...
    --M&B: vanilla values an enemy ACU at a fixed iBaseACUThreat=1000 ("~20 tanks"), calibrated for VANILLA unit costs (~50 mass/tank). M&B units are far cheaper (~24-60 mass), so 1000 threat makes an enemy ACU look like ~40+ M&B tanks -> (a) the bot's army scatters/"flees like roaches" when an enemy ACU advances (the ACU's threat spikes the zone threat past the retreat gates), and (b) the ACU-attack gate (bConsiderAttackingACU in M28Land.lua, which needs ourThreat>=enemyACUThreat) almost never triggers so the bot wont commit to killing it. Lower it for M&B so the army stops panicking AND its existing ACU-hunt logic unlocks (it will then target/charge the enemy ACU once its army nearby is comparable). TUNABLE (400; was 1000; lower toward 200 if still feared, raise if the bot feeds units to a combat-upgraded ACU). Same mechanism as iThreatFactor above (read dynamically inside GetCombatThreatRating, M28UnitInfo.lua ~757/992, and the ACU threat cache set at ~665/690).
    if M28Utilities.IsMBModActive() then M28UnitInfo.iBaseACUThreat = 200 end
    ...
    if not((ScenarioInfo.Options.M28CombinedArmy or 2) == 2) then table.insert(tsNonStandardSettings, 'Combined army setting='..(ScenarioInfo.Options.M28CombinedArmy or 'Nil')..'; Inherit setting='..(ScenarioInfo.Options.M28CAInherit or 'Nil')..'; Engi setting='..(ScenarioInfo.Options.M28CAEngi or 'nil')) end --M&B fix: nil option is the standard (2) everywhere else in M28; only this warning treated nil as non-standard (false positive)
...
function M28BrainCreated(aiBrain)
    ...
    if not(bInitialSetup) then
        bInitialSetup = true
        local LoudCompatibility = import('/mods/M&B/lua/AI/LOUD/M28OtherLOUDCompatibility.lua')
        ...
        if import('/mods/M&B/lua/M28Config.lua').M28RunProfiling then M28Utilities.ErrorHandler('Debut mode is enabled') end
        --LOG('M28 in game 3')
        ...
        M28Chat.SendMessage(oChatBrain, 'LoadingMap', 'Analysing map (M28 v'..import('/mods/M&B/mod_info.lua').version..'), wait a minute', 0, 10000, false)
...
function OverseerManager(aiBrain)
    ...
    while not(aiBrain:IsDefeated()) and not(aiBrain.M28IsDefeated) do
        ...
        --[[if GetGameTimeSeconds() >= 900 and not(bEnabledProfiling) then
             if not(import('/mods/M&B/lua/M28Config.lua').M28RunProfiling) then
                 ForkThread(M28Profiler.ProfilerActualTimePerTick)
                 import('/mods/M&B/lua/M28Config.lua').M28RunProfiling = true
             end
             bEnabledProfiling = true
         end--]]

        --if GetGameTimeSeconds() >= 2700 then import('/mods/M&B/lua/M28Config.lua').M28ShowUnitNames = true end
