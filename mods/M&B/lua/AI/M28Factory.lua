...
local M28Engineer = import('/mods/M&B/lua/AI/M28Engineer.lua')
local NavUtils = M28Utilities.NavUtils
local M28Building = import('/mods/M&B/lua/AI/M28Building.lua')
local M28Air = import('/mods/M&B/lua/AI/M28Air.lua')
local M28Micro = import('/mods/M&B/lua/AI/M28Micro.lua')
local M28ACU = import('/mods/M&B/lua/AI/M28ACU.lua')
local M28Land = import('/mods/M&B/lua/AI/M28Land.lua')
...
--================================================================================
-- M&B: Role-balanced land army roster
-- Doctrine: every role is good for its job (light=fast raid/intercept, medium=
-- universal longest range, heavy=tanky alpha to break static defence, bots=faster +
-- more damage but fragile, experimental=strong at everything, on every tier). M28
-- otherwise scores land units and spams the single highest scorer, ignoring variety.
-- This watches the bot's army, finds the regular role it most lacks, and lifts that
-- role's priority in M28's OWN per-brain table (reftBlueprintPriorityOverride, read by
-- GetBlueprintThatCanBuildOfCategory) so the factory naturally builds the gap-filler.
-- Gated on M&B (M28Utilities.IsMBModActive); vanilla M28 is untouched. The rebalance
-- is pcall-guarded so any fault is inert (no warnings, never crashes the AI).
-- Role is read from the unit's Description (Russian: Лёгкий/Средний/Тяжёлый + English
-- fallbacks) plus EXPERIMENTAL/BOT categories; unit IDs are NOT used (the suffix scheme
-- is inconsistent across factions).
--================================================================================
refiMBRosterRebalanceSeconds = 10   --how often the army mix is reassessed per brain
refiMBRosterPriority = 50           --priority given to the deficit role (beats M28 base scoring)
refiMBRosterMinArmy = 6             --dont micromanage the mix until at least this many regular land units exist
tMBRoleTargets = { light = 0.22, medium = 0.30, heavy = 0.30, bot = 0.18 } --target share of the regular land army (~1.0; exp left to M28's existing handling)
tMBRoleCache = {}        --[sBlueprint] -> role or 'none' (lazy; avoids re-scanning blueprints)
tMBRoleUnitLists = false --built once: [role] -> {sBlueprint,...} factory-buildable land/hover DF units per role
tMBRegularRoles = { 'light', 'medium', 'heavy', 'bot' }
tMBRosterThreadStarted = false --M&B: MUST be declared at file scope — M28 strict mode crashes on read of an undeclared global (was causing the thread to die at line ~195 with 'access to nonexistent global variable tMBRosterThreadStarted')
tMBRosterErrorLogged = false

function GetMBUnitRole(sBlueprint)
    --Classify an M&B land combat unit by role. Returns 'light'|'medium'|'heavy'|'bot'|'exp', or nil.
    if sBlueprint == nil then return nil end
    local cached = tMBRoleCache[sBlueprint]
    if cached ~= nil then return cached == 'none' and nil or cached end
    local sRole = nil
    local bp = __blueprints[sBlueprint]
    if bp then
        local bExp, bBot = false, false
        if bp.Categories then
            for iCat, sCat in bp.Categories do
                if sCat == 'EXPERIMENTAL' then bExp = true
                elseif sCat == 'BOT' then bBot = true end
            end
        end
        local sDesc = string.lower(bp.Description or '')
        --M&B: durability role is PRIMARY (the doctrine: light/medium/heavy is the main axis; bot-vs-tank is just chassis). So check durability keywords FIRST. 'bot' is the fallback for units with no durability keyword (e.g. a bare "Assault Bot"). Otherwise walker factions (Seraphim) collapse entirely to 'bot' and the mix has nothing to boost.
        if bExp then sRole = 'exp'
        elseif string.find(sDesc, 'тяжёл', 1, true) or string.find(sDesc, 'тяжел', 1, true) or string.find(sDesc, 'heavy', 1, true) then sRole = 'heavy'
        elseif string.find(sDesc, 'средн', 1, true) or string.find(sDesc, 'medium', 1, true) then sRole = 'medium'
        elseif string.find(sDesc, 'лёгк', 1, true) or string.find(sDesc, 'легк', 1, true) or string.find(sDesc, 'light', 1, true) then sRole = 'light'
        elseif bBot or string.find(sDesc, 'бот', 1, true) or string.find(sDesc, 'bot', 1, true) then sRole = 'bot' end
    end
    tMBRoleCache[sBlueprint] = sRole or 'none'
    return sRole
end

function BuildMBRoleUnitLists()
    --One-time scan: group every factory-buildable land/hover direct-fire unit by role.
    tMBRoleUnitLists = { light={}, medium={}, heavy={}, bot={}, exp={} }
    for sBlueprint, bp in __blueprints do
        local cats = bp and bp.Categories
        if cats then
            local bLandHover, bMobile, bDF, bFac = false, false, false, false
            for iCat, sCat in cats do
                if sCat == 'LAND' or sCat == 'HOVER' then bLandHover = true
                elseif sCat == 'MOBILE' then bMobile = true
                elseif sCat == 'DIRECTFIRE' then bDF = true
                elseif sCat == 'BUILTBYTIER1FACTORY' or sCat == 'BUILTBYTIER2FACTORY' or sCat == 'BUILTBYTIER3FACTORY' then bFac = true end
            end
            if bLandHover and bMobile and bDF and bFac then
                local sRole = GetMBUnitRole(sBlueprint)
                if sRole and tMBRoleUnitLists[sRole] then table.insert(tMBRoleUnitLists[sRole], sBlueprint) end
            end
        end
    end
end

function MBCountArmyByRole(aiBrain)
    --Count the bot's completed land/hover direct-fire army, grouped by role.
    local tCounts = { light=0, medium=0, heavy=0, bot=0, exp=0 }
    local tUnits = aiBrain:GetListOfUnits(categories.MOBILE * (categories.LAND + categories.HOVER) * categories.DIRECTFIRE, false)
    if tUnits then
        for i, oUnit in tUnits do
            local sRole = GetMBUnitRole(oUnit.UnitId)
            if sRole and tCounts[sRole] ~= nil then tCounts[sRole] = tCounts[sRole] + 1 end
        end
    end
    return tCounts
end

function MBRebalanceRosterPriorities(aiBrain)
    --Lift the priority of the regular role the bot's army most lacks; clear the previous boost.
    if tMBRoleUnitLists == false then BuildMBRoleUnitLists() end
    local tCounts = MBCountArmyByRole(aiBrain)
    local iRegular = tCounts.light + tCounts.medium + tCounts.heavy + tCounts.bot
    local sBoost = nil
    if iRegular >= refiMBRosterMinArmy then
        local iWorstDeficit = 0
        for iRole, sRole in tMBRegularRoles do
            local iTarget = math.floor(tMBRoleTargets[sRole] * iRegular + 0.5)
            local iDeficit = iTarget - tCounts[sRole]
            if iDeficit > iWorstDeficit then iWorstDeficit = iDeficit; sBoost = sRole end
        end
    end
    if M28Profiler.bGlobalDebugOverride then
        local iBoostSize = (sBoost and tMBRoleUnitLists[sBoost] and table.getn(tMBRoleUnitLists[sBoost])) or 0
        LOG('M&B roster: counts L/M/H/Bot/Exp='..tCounts.light..'/'..tCounts.medium..'/'..tCounts.heavy..'/'..tCounts.bot..'/'..tCounts.exp..'; iRegular='..iRegular..'; boosting='..(sBoost or 'none')..'; boostListSize='..iBoostSize)
    end
    if sBoost ~= aiBrain.sMBLastBoostedRole then --log role switches unconditionally (no debug needed) — a handful of lines per game, enough to diagnose from a normal log
        aiBrain.sMBLastBoostedRole = sBoost
        LOG('M28AI: M&B roster now boosting '..(sBoost or 'none')..'; army L/M/H/Bot/Exp='..tCounts.light..'/'..tCounts.medium..'/'..tCounts.heavy..'/'..tCounts.bot..'/'..tCounts.exp..' (iRegular='..iRegular..')')
    end
    local tPrio = aiBrain[reftBlueprintPriorityOverride] or {}
    if aiBrain.tMBBoostedBlueprints then
        for i, sBP in aiBrain.tMBBoostedBlueprints do tPrio[sBP] = nil end
    end
    aiBrain.tMBBoostedBlueprints = {}
    if sBoost and tMBRoleUnitLists[sBoost] then
        for i, sBP in tMBRoleUnitLists[sBoost] do
            tPrio[sBP] = refiMBRosterPriority
            table.insert(aiBrain.tMBBoostedBlueprints, sBP)
        end
    end
    aiBrain[reftBlueprintPriorityOverride] = tPrio
end

function MBRosterThread(aiBrain)
    --Forked from hook/lua/aibrain.lua OnCreateAI for m28/easy/medium brains. OnCreateAI fires BEFORE __blueprints is populated, so wait first, then gate on IsMBModActive (which reads __blueprints['sab9101']); exit cleanly in non-M&B games so we dont leave a dormant loop. pcall so any fault is inert (no warnings, no crash).
    WaitSeconds(refiMBRosterRebalanceSeconds)
    if not M28Utilities.IsMBModActive() then return end
    if not tMBRosterThreadStarted then tMBRosterThreadStarted = true; LOG('M28AI: M&B roster rebalance thread active (role-balanced army mix)') end
    while not (aiBrain.M28IsDefeated) do
        local bOK, sErr = pcall(MBRebalanceRosterPriorities, aiBrain)
        if not bOK and not tMBRosterErrorLogged then tMBRosterErrorLogged = true; LOG('M28AI: M&B roster rebalance error (logged once, feature now inert): '..tostring(sErr)) end
        WaitSeconds(refiMBRosterRebalanceSeconds)
    end
end
...
function GetBlueprintThatCanBuildOfCategory(aiBrain, iCategoryCondition, oFactory, bGetSlowest, bGetFastest, bGetCheapest, iOptionalCategoryThatMustBeAbleToBuild, bIgnoreTechDifferences, iOptionalMaxSkirtSize, bGetMostExpensive)
    ...
    if not(oFactory.CanBuild) then
        ...
        for _, sBlueprint in tBlueprints do
            ...
            if oFactory:CanBuild(sBlueprint) == true and (not(M28Utilities.IsMBModActive()) or not(M28UnitInfo.IsUnitRestricted(sBlueprint, iArmyIndex))) then
                ...
                if bCanBuildRequiredCategory then
                    ...
                    if not(iOptionalMaxSkirtSize) or M28UnitInfo.GetBuildingSize(sBlueprint) <= iOptionalMaxSkirtSize then
                        ...
                        if bIgnoreTechDifferences then iCurrentTech = 1
                            ...
                            --M&B: EXPERIMENTAL is a build-method (NEEDMOBILEBUILD), not a tier; tiers span T1-T4, so dont force EXPERIMENTAL=>4. Vanilla tech detection restored.
        ...
        for _, sBlueprint in tValidBlueprints do
            bIsValid = false
            --M&B: EXPERIMENTAL is a build-method, not a tier; reverted the EXPERIMENTAL=>4 override (wrong premise).
            if EntityCategoryContains(categories.TECH3 + categories.EXPERIMENTAL, sBlueprint) then iCurrentTech = 3
            elseif EntityCategoryContains(categories.TECH2, sBlueprint) then iCurrentTech = 2
            else iCurrentTech = 1
            end
        ...
        --M&B: factory unit rotation within a tier. M28 spams a single unit because (a) IsUnitRestricted is blind to M&B per-army research-locks and wrongly filters candidates, and (b) the roster's reftBlueprintPriorityOverride concentrates on one unit so only 1 candidate reaches the random pick below. For M&B factories: ignore IsUnitRestricted (CanBuild is authoritative - it sees per-army locks) AND priority; collect all CanBuild candidates, take the highest tier, rotate by the factory's build-count so it cycles through available tanks/bots (A->B->C->A) instead of spamming one. Scoped to factory builders (engineer structure/exp picks use the normal path).
        if M28Utilities.IsMBModActive() and oFactory.UnitId and EntityCategoryContains(M28UnitInfo.refCategoryFactory, oFactory.UnitId) then
            local tMNBBest = {}
            local iMNBHighTech = 1
            for _, sBlueprint in tBlueprints do
                if oFactory:CanBuild(sBlueprint) == true then
                    local iMNBTech = 1
                    --M&B: EXPERIMENTAL factory units are the mod's 'T4' (unlocked at study 9400). Make them tier 4 (above T3) so once 9400 is done the factory builds T4-only (T4 dominates the rotation). Before 9400 they're CanBuild=false (locked) so T3 is still built. NOTE: categories.TECH4 does NOT exist in FA (only TECH1/2/3) — using it crashes EntityCategoryContains.
                    if EntityCategoryContains(categories.EXPERIMENTAL, sBlueprint) then iMNBTech = 4
                    elseif EntityCategoryContains(categories.TECH3, sBlueprint) then iMNBTech = 3
                    elseif EntityCategoryContains(categories.TECH2, sBlueprint) then iMNBTech = 2 end
                    if iMNBTech > iMNBHighTech then iMNBHighTech = iMNBTech; tMNBBest = {sBlueprint}
                    elseif iMNBTech == iMNBHighTech then table.insert(tMNBBest, sBlueprint) end
                end
            end
            --M&B (patch #15b, 2026-07-16): weighted tank mix 1 light : 5 medium : 1 heavy. Problem: the even rotation below gives ~1:1:1 across the tier; in M&B the HEAVY tank has the SHORTEST range (T1: 22 vs medium 32), so in the bot's clumped army only mediums reach the enemy - the player spams mediums and kills the bot at range. Fix: among the DIRECTFIRE-land candidates of the current tier (tMNBBest), classify by BuildCostMass (cheap=light, middle=medium, expensive=heavy), then a 7-build cycle: slot0=light, slots1-5=medium, slot6=heavy. Relative mass => no hardcoded IDs/thresholds, auto-adapts to faction/tier. Only DIRECTFIRE land (engineers/scouts/arty/AA/air/naval pass through to the even rotation below); empty basket => fallback to medium then to the even rotation.
            if table.getn(tMNBBest) >= 2 then
                local tMNBTanks = {}
                for _, sBlueprint in tMNBBest do
                    if EntityCategoryContains(categories.LAND * categories.MOBILE * categories.DIRECTFIRE, sBlueprint)
                       and not EntityCategoryContains(categories.ENGINEER + categories.SCOUT + categories.ANTIAIR, sBlueprint) then
                        table.insert(tMNBTanks, sBlueprint)
                    end
                end
                --M&B fix (2026-07-16): if the tier has only 1 DIRECTFIRE tank (T3 for every faction - BRNT3BT0/BROT3BT2/BRMT3HT/BRPT3BT; T2 Seraphim BRPT2BTBOT), the weighted 1:5:1 block below needs >=2 tanks and is skipped -> the even rotation at ~458 then mixes the single tank with arty/FIELDENGINEER/SUBCOMMANDER (all in refCategoryLandCombat) = "solyanka". So if exactly 1 tank, spam it cleanly instead of the mix. On T1/T2-non-Seraphim (>=2 tanks) nothing changes - 1:5:1 as before.
                if table.getn(tMNBTanks) == 1 then
                    if bDebugMessages == true then LOG(sFunctionRef..': M&B single tank in tier - spamming '..tMNBTanks[1]..' (no 1:5:1 possible with 1 tank)') end
                    M28Profiler.FunctionProfiler(sFunctionRef, M28Profiler.refProfilerEnd)
                    return tMNBTanks[1]
                end
                if table.getn(tMNBTanks) >= 2 then
                    table.sort(tMNBTanks, function(a, b)
                        return (tAllBlueprints[a] and tAllBlueprints[a].Economy and tAllBlueprints[a].Economy.BuildCostMass or 0) < (tAllBlueprints[b] and tAllBlueprints[b].Economy and tAllBlueprints[b].Economy.BuildCostMass or 0)
                    end)
                    local tMNBLight, tMNBMedium, tMNBHeavy = {}, {}, {}
                    if table.getn(tMNBTanks) >= 3 then
                        tMNBLight = { tMNBTanks[1] }
                        tMNBHeavy = { tMNBTanks[table.getn(tMNBTanks)] }
                        for iMNBTankIdx = 2, table.getn(tMNBTanks) - 1 do
                            table.insert(tMNBMedium, tMNBTanks[iMNBTankIdx])
                        end
                    else
                        --Only 2 types (tier without a light/heavy): treat the cheaper as 'medium' (the spam backbone), the pricier as heavy
                        tMNBMedium = { tMNBTanks[1] }
                        tMNBHeavy = { tMNBTanks[2] }
                    end
                    local iMNBSlot = math.mod(oFactory[refiTotalBuildCount] or 0, 7)
                    local tMNBPick = tMNBMedium
                    if iMNBSlot == 0 and table.getn(tMNBLight) > 0 then tMNBPick = tMNBLight
                    elseif iMNBSlot == 6 and table.getn(tMNBHeavy) > 0 then tMNBPick = tMNBHeavy end
                    if table.getn(tMNBPick) > 0 then
                        local sMNBPick = tMNBPick[math.mod(oFactory[refiTotalBuildCount] or 0, table.getn(tMNBPick)) + 1]
                        if bDebugMessages == true then LOG(sFunctionRef..': M&B weighted tank mix - picking '..sMNBPick..' slot='..iMNBSlot..' light='..table.getn(tMNBLight)..' medium='..table.getn(tMNBMedium)..' heavy='..table.getn(tMNBHeavy)) end
                        M28Profiler.FunctionProfiler(sFunctionRef, M28Profiler.refProfilerEnd)
                        return sMNBPick
                    end
                end
            end
            if table.getn(tMNBBest) > 1 then
                local iMNBIdx = math.mod((oFactory[refiTotalBuildCount] or 0), table.getn(tMNBBest)) + 1
                M28Profiler.FunctionProfiler(sFunctionRef, M28Profiler.refProfilerEnd)
                return tMNBBest[iMNBIdx]
            elseif table.getn(tMNBBest) == 1 then
                M28Profiler.FunctionProfiler(sFunctionRef, M28Profiler.refProfilerEnd)
                return tMNBBest[1]
            end
        end
...
function GetBlueprintToBuildForLandFactory(aiBrain, oFactory)
    ...
    --M&B fix (2026-07-16, REVISED per user): factory upgrade timing. Originally (#41) EVERY land factory upgraded at once the moment 6 extractors hit T2+ -> all factories upgraded in a clump WHILE the remaining mexes were still upgrading -> mass sinkhole ("factories upgrade in a clump and before mexes"). Two guards now: (1) gate on a FRACTION of extractors upgraded (>=70% of all mexes are T2/T3, min 4) so mexes finish FIRST - they are the mass income the factory upgrade depends on; (2) per-brain cooldown (one factory upgrade per ~30s) so they DONT all upgrade at once (staggered). Gated on M&B; vanilla unchanged.
    ...
    --M&B: doctrine (user) - SPAM tanks/bots (land combat) until we have ~200 ALIVE, to overwhelm players who spam tanks. The vanilla factory balance + the exp-triggered bDontGetCombat (refiConstructedExperimentalCount>0 makes the factory build support/engineers instead of combat) left the bot with ~2 tanks and a pile of engineers/arty/AA - defenseless against a tank-rush. So under the threshold force a land-combat unit and short-circuit the rest of the support/exp logic. GetBlueprintThatCanBuildOfCategory still respects CanBuild (tier) and runs the M&B rotation (patch #15). Threshold on ALIVE count so losses are replenished.
    if M28Utilities.IsMBModActive() and aiBrain:GetCurrentUnits(M28UnitInfo.refCategoryLandCombat) < 200 then
        --M&B fix (2026-07-16, REVISED per user feedback): engineers FIRST and in a BURST (NOT throttled), before scouts/arty. Previously the scout carve-out (4 scouts in a row) + the 1-in-3 engineer throttle meant the FIRST engineer out of a land factory was only the ~6th unit built - by then the air factory was already producing engineers, so the land factory contributed nothing ("no difference"). Now: T1 engineers come out FIRST, up to an alive cap, so they beat the air factory online and give the bot early expansion/reclaim. After the pool is filled, scouts/arty/tanks follow.
        if iFactoryTechLevel == 1 then
            local iMNBEngiCap = 10 --TUNABLE (was 6; user: 6 too few to take mexes in time)
            local iMNBCurEngis = aiBrain:GetCurrentUnits(M28UnitInfo.refCategoryEngineer * categories.TECH1)
            if iMNBCurEngis < iMNBEngiCap then
                local sMNBEngiBP = GetBlueprintThatCanBuildOfCategory(aiBrain, M28UnitInfo.refCategoryEngineer, oFactory)
                if sMNBEngiBP then
                    if bDebugMessages == true then LOG(sFunctionRef..': M&B tank-spam - letting T1 engi through FIRST; cur T1 engis='..iMNBCurEngis..'/'..iMNBEngiCap) end
                    M28Profiler.FunctionProfiler(sFunctionRef, M28Profiler.refProfilerEnd)
                    return sMNBEngiBP
                end
            end
        end
        --M&B: scouts come AFTER the engineer pool (cheap; just a few for vision)
        local iMNBScoutCap = 4
        if aiBrain:GetCurrentUnits(M28UnitInfo.refCategoryLandScout) < iMNBScoutCap then
            local sMNBScoutBP = GetBlueprintThatCanBuildOfCategory(aiBrain, M28UnitInfo.refCategoryLandScout, oFactory)
            if sMNBScoutBP then
                if bDebugMessages == true then LOG(sFunctionRef..': M&B tank-spam doctrine - letting a scout through; cur land scouts='..aiBrain:GetCurrentUnits(M28UnitInfo.refCategoryLandScout)..' (cap '..iMNBScoutCap..')') end
                M28Profiler.FunctionProfiler(sFunctionRef, M28Profiler.refProfilerEnd)
                return sMNBScoutBP
            end
        end
        --M&B fix (2026-07-16, REVISED per user): limited artillery AND gated behind study 9100. User: "arty is expensive and not needed at the very start - tie it to 9100, plus cap it". So: build NO mobile arty until study 9100 (MNB_TechUnlocked[2], the 'second half of T1' research) completes; after that, build only a handful (alive cap 5, throttled 1-in-4) for sieging defenses. T1 mobile arty is part of refCategoryLandCombat so without this gate the rotation built it from unit #1.
        local iMNBArtiCap = 5 --TUNABLE
        if aiBrain.MNB_TechUnlocked and aiBrain.MNB_TechUnlocked[2] and aiBrain:GetCurrentUnits(M28UnitInfo.refCategoryIndirect) < iMNBArtiCap and math.mod(oFactory[refiTotalBuildCount] or 0, 4) == 2 then
            local sMNBArtiBP = GetBlueprintThatCanBuildOfCategory(aiBrain, M28UnitInfo.refCategoryIndirect, oFactory)
            if sMNBArtiBP then
                if bDebugMessages == true then LOG(sFunctionRef..': M&B tank-spam - letting arty through; cur arty='..aiBrain:GetCurrentUnits(M28UnitInfo.refCategoryIndirect)..'/'..iMNBArtiCap) end
                M28Profiler.FunctionProfiler(sFunctionRef, M28Profiler.refProfilerEnd)
                return sMNBArtiBP
            end
        end
    ...
    --M&B doctrine: factory should pump COMBAT, not support. (1) AA ONLY if the enemy actually has air-to-ground threat — otherwise M28 wastes factory cycles on MAA that a human player easily swats. (2) ~2 land scouts is plenty; stop queuing more. Inserted AFTER both flags are finalised (bDontConsiderLandScouts at ~1440, bDontConsiderBuildingMAA above) and BEFORE every build-decision use of them below. Gated on M&B; vanilla M28 untouched.
    if M28Utilities.IsMBModActive() then
        if (M28Team.tTeamData[iTeam][M28Team.refiEnemyAirToGroundThreat] or 0) <= 0 and (tLZTeamData[M28Map.refiEnemyAirToGroundThreat] or 0) <= 0 then
            bDontConsiderBuildingMAA = true
        end
        if not(bDontConsiderLandScouts) and aiBrain:GetCurrentUnits(M28UnitInfo.refCategoryLandScout) >= 2 then
            bDontConsiderLandScouts = true
        end
    end
    ...
    function ConsiderUpgrading()
        --M&B doctrine (user, 2026-07-16): upgrade ALL land factories in parallel when a tier opens, not just one at a time. The vanilla throttle below (count of active HQ upgrades + available-factory-in-LZ checks) is built for vanilla's engineer-assist economy; in M&B mass is the only constraint and extractors self-upgrade in parallel, so every land factory should step up a tier together. Skip the throttle for M&B - the #12 '6 upgraded extractors' gate in UpgradeUnit still guards the economy. Mirrors the air-factory ConsiderUpgrading (which has no throttle).
        if M28Utilities.IsMBModActive() then
    ...
    if aiBrain[M28Overseer.refbCloseToUnitCap] and (iFactoryTechLevel < math.min(3, M28Team.tTeamData[iTeam][M28Team.subrefiHighestFriendlyLandFactoryTech]) or (bHaveLowMass and not(tLZTeamData[M28Map.subrefbEnemiesInThisOrAdjacentLZ]))) then
        ...
        if iFactoryTechLevel == 1 and GetGameTimeSeconds() <= 480 and tLZTeamData[M28Map.subrefLZbCoreBase] and not(M28Map.bIsLowMexMap) and (not(tLZTeamData[M28Map.subrefbDangerousEnemiesInThisLZ]) or (oFactory[refiTotalBuildCount] <= 4 and M28UnitInfo.GetUnitLifetimeCount(oFactory) <= 2)) and (not (aiBrain[M28Map.refbCanPathToEnemyBaseWithLand]) or M28Utilities.GetDistanceBetweenPositions(tLZTeamData[M28Map.reftClosestEnemyBase], tLZData[M28Map.subrefMidpoint]) >= 450) then
            ...
            if categories.RESEARCHLOCKEDTECH1 then iLCWanted = math.min(iLCWanted, 6) end --M&B: fewer early engineers (was up to 12) so mass isnt drained and the AI can actually build an army
...
function DetermineWhatToBuild(aiBrain, oFactory)
    ...
    elseif EntityCategoryContains(categories.RESEARCHCENTRE, oFactory.UnitId) then
        sBPIDToBuild = GetBlueprintToBuildForResearchCentre(aiBrain, oFactory)
    else
...
function GetBlueprintToBuildForAirFactory(aiBrain, oFactory)
    ...
    if bHaveLowPower or (bHaveLowMass and aiBrain[M28Overseer.refbPrioritiseLand] and ((M28Team.tTeamData[iTeam][M28Team.subrefiHighestEnemyAirTech] == 3 and M28Team.tAirSubteamData[iAirSubteam][M28Team.subrefiOurAirAAThreat] >= 1500) or (iFactoryTechLevel >= M28Team.tTeamData[iTeam][M28Team.subrefiHighestEnemyAirTech] and not(M28Team.tAirSubteamData[iAirSubteam][M28Team.refbFarBehindOnAir]) and M28Team.tAirSubteamData[iAirSubteam][M28Team.subrefiOurAirAAThreat] >= 500))) then
        ...
        --M&B: low-power mode is almost always active here (research labs draw energy), so the normal-power air-scout builders are never reached and the bot builds ~0 air scouts -> goes blind from the air. Mirror a simple scout builder: if we have fewer than 2 scouts per active brain and arent energy-stalling, build one. ManageAirScouts will send built scouts to scout the enemy. Gated on M&B; vanilla unchanged.
        if M28Utilities.IsMBModActive() and not(M28Team.tTeamData[iTeam][M28Team.subrefbTeamIsStallingEnergy]) then
            if not(iAirSubteamAirScouts) then iAirSubteamAirScouts = M28Conditions.GetNumberOfConstructedUnitsInAirSubteam(iAirSubteam, M28UnitInfo.refCategoryAirScout) end
            local iMNBAirScoutCap = math.max(2, 2 * (M28Team.tTeamData[iTeam][M28Team.subrefiActiveM28BrainCount] or 1))
            if (iAirSubteamAirScouts or 0) < iMNBAirScoutCap then
                if bDebugMessages == true then LOG(sFunctionRef..': M&B low-power air scout builder, iAirSubteamAirScouts='..tostring(iAirSubteamAirScouts)..' cap='..iMNBAirScoutCap) end
                if ConsiderBuildingCategory(M28UnitInfo.refCategoryAirScout) then return sBPIDToBuild end
            end
        end
        ...
        --M&B: vanilla triggers emergency ASF on ANY enemy air in the LZ (even a lone scout/interceptor), so the air factory never gets past this block to build bombers/gunships and just spams interceptors. In M&B only treat enemy interceptors as an emergency when the threat is real (>=300); enemy air-to-ground (bombers/gunships) still always triggers ASF. Vanilla M28 unchanged. Gated on M&B.
        local iMNBEnemyAirToGround = tLZTeamData[M28Map.refiEnemyAirToGroundThreat] or 0
        local iMNBEnemyAirAA = tLZTeamData[M28Map.refiEnemyAirAAThreat] or 0
        local bMNBNeedEmergencyASF = (iMNBEnemyAirToGround > 0) or (M28Utilities.IsMBModActive() and iMNBEnemyAirAA >= 300) or (not(M28Utilities.IsMBModActive()) and iMNBEnemyAirAA > 0)
        if bMNBNeedEmergencyASF then
            if ConsiderBuildingCategory(M28UnitInfo.refCategoryAirAA) then return sBPIDToBuild end
        end
...
function GetBlueprintToBuildForResearchCentre(aiBrain, oFactory)
    ...
    local tTierResearchIds = { RESEARCHLOCKEDTECH1 = true, TECH2 = true, TECH3 = true, EXPERIMENTAL = true, TECH4 = true }
    local tBlueprints = EntityCategoryGetUnitList(categories.BUILTBYRESEARCH)
    local sTierPick, sOtherPick
    if tBlueprints then
        for _, sBPID in tBlueprints do
            if oFactory:CanBuild(sBPID) then
                local oBP = __blueprints[sBPID]
                if oBP and tTierResearchIds[oBP.ResearchId] then
                    sTierPick = sTierPick or sBPID
                else
                    sOtherPick = sOtherPick or sBPID
                end
            end
        end
    end
    ...
    if M28Utilities.IsMBModActive() and tBlueprints then
        ...
        for _, sBPID in tBlueprints do
            if oFactory:CanBuild(sBPID) then
                local sMNBNum = string.sub(sBPID, 4) --drop the ser/sar/srr/ssr prefix -> '9100','910600',...
                local iMNBRank = tMNBOrder[sMNBNum]
                if iMNBRank and iMNBRank < iMNBChosenRank then
                    iMNBChosenRank = iMNBRank
                    sMNBChosen = sBPID
                end
            end
        end
        if sMNBChosen then return sMNBChosen end
    end
    return sTierPick or sOtherPick
end
