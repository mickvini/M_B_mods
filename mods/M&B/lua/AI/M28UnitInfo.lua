...
refCategoryResearchFactory = categories.RESEARCHCENTRE * categories.FACTORY * categories.STRUCTURE
refCategoryFactory = refCategoryLandFactory + refCategoryAirFactory + refCategoryNavalFactory + refCategoryResearchFactory
...
function GetBlueprintFromID(sBlueprintID)
    ...
    if sBlueprintID == nil then return nil end --M&B: guard against nil (a caller passing nil caused 'bad argument #1 to lower (string expected, got nil)' here)
                    ...
                    if oBP.Physics.BuildOnLayerCaps then
                        ...
                        if oBP.Physics.BuildOnLayerCaps['LAYER_Water'] or (oBP.General.Icon == 'amph' and (M28Utilities.bLoudModActive or M28Utilities.bQuietModActive)) then
                            ...
                            if bDebugMessages == true then
                                LOG(sFunctionRef..': Recorded that PD unit '..sUnitId..' can be built on both land and water, list of blueprints that meet the engineer build torp luancher cat='..repru(EntityCategoryGetUnitList(import('/mods/M&B/lua/AI/M28Engineer.lua').tiActionCategory[import('/mods/M&B/lua/AI/M28Engineer.lua').refActionBuildTorpLauncher]))..'; List of unit blueprints in refCategoryHoverPD='..repru(EntityCategoryGetUnitList(refCategoryHoverPD)))
                            end
                        end
                    end
...
function DisableUnitJamming(oUnit)
    if bDontConsiderCombinedArmy or oUnit.M28Active then
        pcall(oUnit.OnScriptBitSet, oUnit, 2) --M&B-guard: some units have incomplete intel blueprints; OnScriptBitSet->EnableUnitIntel does arithmetic on a nil field (unit.lua:1820), crashing M28's PauseOrUnpauseEnergyUsage loop (saw 138x). pcall swallows it so the energy loop keeps running.
    end
end
function EnableUnitJamming(oUnit)
    if bDontConsiderCombinedArmy or oUnit.M28Active then
        pcall(oUnit.OnScriptBitClear, oUnit, 2) --M&B-guard: see DisableUnitJamming (unit.lua:1820 nil arithmetic in EnableUnitIntel)
    end
end
...
function PauseOrUnpauseMassUsage(oUnit, bPauseNotUnpause, iOptionalTeam, iPausePriority)
    ...
    if bDontConsiderCombinedArmy or oUnit.M28Active then
        ...
        if IsUnitValid(oUnit) and oUnit:GetFractionComplete() == 1 and oUnit.SetPaused and (not(oUnit[refbCampaignNeverPause]) or not(bPauseNotUnpause)) then
            ...
            if (not(bPauseNotUnpause) or not(oUnit:IsPaused())) and not(categories.RESEARCHLOCKEDTECH1 and bPauseNotUnpause and EntityCategoryContains(refCategoryFactory, oUnit.UnitId)) and (not(EntityCategoryContains(refCategoryFactory, oUnit.UnitId)) or (oUnit.GetWorkProgress and oUnit:GetWorkProgress() > 0 and oUnit:GetWorkProgress() < 1) or (oUnit:IsPaused() and not(bPauseNotUnpause))) then
...
function PauseOrUnpauseEnergyUsage(oUnit, bPauseNotUnpause, bExcludeProduction, iOptionalTeam, iPausePriority)
    ...
    if bDontConsiderCombinedArmy or oUnit.M28Active then
        ...
        if bDebugMessages == true then
            ...
            if oUnit.GetWorkProgress then LOG(sFunctionRef..': Unit work progress='..oUnit:GetWorkProgress()..'; Unit fraction complete='..oUnit:GetFractionComplete()..'; Is arti template nil='..tostring(oUnit[import('/mods/M&B/lua/AI/M28Building.lua').reftArtiTemplateRefs] == nil)) end
        end
        if IsUnitValid(oUnit) and oUnit:GetFractionComplete() == 1 and oUnit.SetPaused and (not(oUnit[refbCampaignNeverPause]) or not(bPauseNotUnpause)) then
            ...
            if not(bExcludeProduction) or bPauseNotUnpause then
                ...
                --M&B: dont pause factories on energy stall - in this mod factories always consume energy (maintenance), so pausing doesnt save energy and only stalls production. Unpause is still allowed.
                if oUnit.SetPaused and (not(bPauseNotUnpause) or not(oUnit:IsPaused())) and not(categories.RESEARCHLOCKEDTECH1 and bPauseNotUnpause and EntityCategoryContains(refCategoryFactory, oUnit.UnitId)) and (not(EntityCategoryContains(refCategoryFactory, oUnit.UnitId)) or (oUnit.GetWorkProgress and oUnit:GetWorkProgress() > 0 and oUnit:GetWorkProgress() < 1)) then
                    if oUnit.UnitId == 'xsb2401'  and bPauseNotUnpause then M28Utilities.ErrorHandler('Pausing Yolona') end
                    if bDebugMessages == true then LOG(sFunctionRef..': About to set paused to '..tostring(bPauseNotUnpause)..' for unit '..oUnit.UnitId..GetUnitLifetimeCount(oUnit)..'; Unit state='..GetUnitState(oUnit))
                        if oUnit.GetWorkProgress then LOG(sFunctionRef..': Unit work progress='..oUnit:GetWorkProgress()) end
                    end
...
function GetFactoryType(oUnit)
    local M28Factory = import('/mods/M&B/lua/AI/M28Factory.lua') --Putting this at the top crashes the game
...
function CloakUnit(oUnit, bDontClearExistingOrders, bUpdateTracking)
    if bDontConsiderCombinedArmy or oUnit.M28Active then
        if not(bDontClearExistingOrders) then import('/mods/M&B/lua/AI/M28Orders.lua').IssueTrackedClearCommands(oUnit) end
