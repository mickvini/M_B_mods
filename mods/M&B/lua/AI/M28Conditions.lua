...
local M28Logic = import('/mods/M&B/lua/AI/M28Logic.lua')
...
function IsCivilianBrain(aiBrain)
    ...
    if aiBrain.M28IsCivilian == nil then
        if M28Utilities.bSteamActive then import('/mods/M&B/lua/AI/Steam/SteamCompatibility.lua').OtherSteamCompatibilityInformation() end
...
function HaveEnoughThreatToAttack(iPlateau, iLandZone, tLZData, tLZTeamData, iOurCombatThreat, iEnemyCombatThreat, iFirebaseThreatAdjust, bHaveSignificantCombatCloserToFirebase, iTeam, iOptionalOverrideDefaultThreatRatioWanted, bOptionalUseSlightlyLowerThreatRatio)
    ...
    elseif bOptionalUseSlightlyLowerThreatRatio then
        ...
        --M&B: vanilla needs the army to be 1.4x stronger than the enemy before committing (so the army often won't get an attack order and drifts back to the rear rally point -> looks cowardly / "retreats even when stronger"). M&B units are far cheaper by mass and a cheap T1 swarm inflates perceived enemy threat, so the bar is too high. Lower it so the bot commits at near-parity (willing to fight slightly outnumbered). 0.9 = fight when our threat is at least 90% of enemy's. TUNABLE: raise toward 1.0 if the bot starts throwing armies away; lower toward 0.8 if still passive. See M&B patch audit.
        if M28Utilities.IsMBModActive() then iDefaultThreatRatioWanted = 0.9 end
    end
...
function PrioritiseSniperBots(tLZData, iTeam, tLZTeamData, iPlateau, iLandZone, bHaveAeonOrSeraFactoryInZoneOverride)
    ...
    if tLZTeamData[M28Map.subrefiNearbyEnemyLongRangeDFThreat] < 2600 then --If enemy has lots of sniperbots then we probably lose in a fight (if we check for them), but main purpose of this is ravagers since they now outrange sniperbots in FAF, so better off going for t3 mobile arti
        ...
        if not(bDangerousExperimentalOrACUThreat) and M28Team.tTeamData[iTeam][M28Team.refbEnemyHasDangerousACU] and M28Team.tTeamData[iTeam][M28Team.subrefiTeamGrossMass] >= 10 and (M28Team.tTeamData[iTeam][M28Team.subrefiTeamGrossMass] >= 30 or tLZTeamData[M28Map.subrefMexCountByTech][3] > 0 or tLZTeamData[M28Map.subrefMexCountByTech][2] >= 4) and not(bEnemyHasLongRangeThreat) and M28Utilities.IsTableEmpty(tLZTeamData[M28Map.subreftoAllNearbyEnemyT2ArtiUnits]) then
            ...
            if bDebugMessages == true then LOG(sFunctionRef..': Deciding if have approaching ACU threat with a +100 dist mod, iACUThreat='..iACUThreat..'; Build tech='..(oACU[import('/mods/M&B/lua/AI/M28ACU.lua').refiBuildTech] or 'nil')..'; Closest brain land fac tech='..ArmyBrains[tLZTeamData[M28Map.reftiClosestFriendlyM28BrainIndex]][M28Economy.refiOurHighestLandFactoryTech]) end
            if iACUThreat >= 2500 then
                if (not(EntityCategoryContains(categories.UEF, oACU.UnitId)) or oACU[import('/mods/M&B/lua/AI/M28ACU.lua').refiBuildTech] or 0) < 3 or ArmyBrains[tLZTeamData[M28Map.reftiClosestFriendlyM28BrainIndex]][M28Economy.refiOurHighestLandFactoryTech] < 3 then
...
function IsTeamCoalition(iTeam)
    ...
    return import('/mods/M&B/lua/AI/M28Chat.lua').IsTeamCoalition(iTeam)
end
