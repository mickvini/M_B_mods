...
local EXCEMPArrayBeam02 = import('/mods/M&B/lua/EXBlackOpsweapons.lua').EXCEMPArrayBeam02 
local EXCEMPArrayBeam03 = import('/mods/M&B/lua/EXBlackOpsweapons.lua').EXCEMPArrayBeam03 
...
ERL0001 = Class(CWalkingLandUnit) {
    ...
    Weapons = {
        ...
        EXEMPArray01 = Class(EXCEMPArrayBeam01) {
            OnWeaponFired = function(self)
				...
				if self.targetaquired then
					...
					table.insert( self.unit.EMPArrayEffects01, AttachBeamEntityToEntity(self.unit, 'EMP_Array_Beam_01', self.unit, 'EMP_Array_Muzzle_01', self.unit:GetArmy(), '/mods/M&B/effects/emitters/excemparraybeam02_emit.bp') )
					table.insert( self.unit.EMPArrayEffects01, AttachBeamEntityToEntity(self.unit, 'EMP_Array_Beam_02', self.unit, 'EMP_Array_Muzzle_02', self.unit:GetArmy(), '/mods/M&B/effects/emitters/excemparraybeam02_emit.bp') )
					table.insert( self.unit.EMPArrayEffects01, AttachBeamEntityToEntity(self.unit, 'EMP_Array_Beam_03', self.unit, 'EMP_Array_Muzzle_03', self.unit:GetArmy(), '/mods/M&B/effects/emitters/excemparraybeam02_emit.bp') )
        ...
        MNB_AutoOC = Class(CDFOverchargeWeapon) {
            ...
            OnGotTarget = function(self)
                if self:MNB_CanAutoOC() then
                    CDFOverchargeWeapon.OnGotTarget(self)
                else
                    self:OnDisableWeapon()
                end
            end,
    ...
    WarpInEffectThread = function(self)
        ...
        WaitSeconds(2.1)
        self:SetMesh('/mods/M&B/modAcu/erl0001/ERL0001_PhaseShield_mesh', true)
...
--                                    LOG(unit.MarkLevel[8])
    ...
    EXRegenBuffThread = function(self)
		...
		if self.wcEMP03 then
			self.regenammount = self.regenammount + EXBP.Enhancements.EXPowerBooster.NewRegenRate
		elseif self.wcEMP02 then
			self.regenammount = self.regenammount + EXBP.Enhancements.EXImprovedCapacitors.NewRegenRate
		elseif self.wcEMP01 then
			self.regenammount = self.regenammount + EXBP.Enhancements.EXEMPArray.NewRegenRate
		end
		if self.wcMasor03 then
			self.regenammount = self.regenammount + EXBP.Enhancements.EXAdvancedEmitterArray.NewRegenRate
		elseif self.wcMasor02 then
			self.regenammount = self.regenammount + EXBP.Enhancements.EXImprovedCoolingSystem.NewRegenRate
		elseif self.wcMasor01 then
			self.regenammount = self.regenammount + EXBP.Enhancements.EXMasor.NewRegenRate
		end
		if self.wcDisruptor01 then
			self.regenammount = self.regenammount + EXBP.Enhancements.EXRipperBooster.NewRegenRate
		end
    end,
    ...
    WeaponConfigCheck = function(self)
		...
		if not self.wcBuildMode and not self.wcOCMode then
			...
			elseif self.wcEMP03 then
				...
				wepAntiMatter06:ChangeMaxRadius(50)
				local wepAntiMatter06 = self:GetWeaponByLabel('EXEMPShot03')
				wepAntiMatter06:ChangeMaxRadius(60)
			...
			if self.wcMasor02 then
				...
				local wepGattling02 = self:GetWeaponByLabel('EXMLG02')
				wepGattling02:ChangeMaxRadius(32)
			else
				self:SetWeaponEnabledByLabel('EXMLG02', false)
			end
			if self.wcMasor03 then
    ...
    --the maser laser line) skip the search entirely and fire at the gun's target,
    --because any turret turn would break their fire stream.
    MNB_AutoOCTry = function(self, gunTarget)
        ...
        if self.wcMasor01 or self.wcMasor02 or self.wcMasor03 then
