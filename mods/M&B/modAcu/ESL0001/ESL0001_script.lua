...
local SeraACURapidWeapon = import('/mods/M&B/lua/EXBlackOpsweapons.lua').SeraACURapidWeapon 
local SeraACUBigBallWeapon = import('/mods/M&B/lua/EXBlackOpsweapons.lua').SeraACUBigBallWeapon 
...
ESL0001 = Class( SWalkingLandUnit ) {
    ...
    Weapons = {
        ...
        MNB_AutoOC = Class(SDFChronotronOverChargeCannonWeapon) {
            ...
            OnGotTarget = function(self)
                if self:MNB_CanAutoOC() then
                    SDFChronotronOverChargeCannonWeapon.OnGotTarget(self)
                else
            ...
            OnEnableWeapon = function(self)
                ...
                --M&B: break the cloak on firing, exactly like the manual OC shot
...
--                                    LOG(unit.MarkLevel[8])
    ...
    WeaponConfigCheck = function(self)
		...
		if not self.wcBuildMode and not self.wcOCMode then
			...
			if self.wcRapid01 then
				...
				local wepGattling01 = self:GetWeaponByLabel('EXRapidCannon01')
				wepGattling01:ChangeMaxRadius(40)
			else
				self:SetWeaponEnabledByLabel('EXRapidCannon01', false)
			end
			if self.wcRapid02 then
				...
				local wepGattling02 = self:GetWeaponByLabel('EXRapidCannon02')
				wepGattling02:ChangeMaxRadius(50)
			else
				self:SetWeaponEnabledByLabel('EXRapidCannon02', false)
			end
			if self.wcRapid03 then
				...
				local wepGattling03 = self:GetWeaponByLabel('EXRapidCannon03')
				wepGattling03:ChangeMaxRadius(60)
			else
				self:SetWeaponEnabledByLabel('EXRapidCannon03', false)
			end
			if self.wcAA01 then
    ...
    --the rapid cannon line) skip the search entirely and fire at the gun's target,
    ...
    MNB_AutoOCTry = function(self, gunTarget)
        ...
        if self.wcRapid01 or self.wcRapid02 or self.wcRapid03 then
    ...
    CreateEnhancement = function(self, enh)
        ...
        elseif enh =='EXImprovedContainmentBottle' then
			...
			local wepChronotron = self:GetWeaponByLabel('ChronotronCannon')
            wepChronotron:ChangeMaxRadius(50)
			self.wcBigBall01 = false
        ...
        elseif enh =='EXPowerBooster' then
			...
			local wepChronotron = self:GetWeaponByLabel('ChronotronCannon')
            wepChronotron:ChangeMaxRadius(60)
            self.wcBigBall01 = false
			self.wcBigBall02 = false
