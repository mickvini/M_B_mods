...
local AeonACUPhasonLaser = import('/mods/M&B/lua/EXBlackOpsweapons.lua').AeonACUPhasonLaser 
local AIFQuasarAntiTorpedoWeapon = AWeapons.AIFQuasarAntiTorpedoWeapon
local EXCEMPArrayBeam01 = import('/mods/M&B/lua/EXBlackOpsweapons.lua').EXCEMPArrayBeam01 
...
local EXQuantumMaelstromWeapon = Class(Weapon) {
    ...
    OnFire = function(self)
        -- Damage is applied by EAL0001:MaelstromAuraThread (5 Hz DoT aura).
    end,
}
    ...
    Weapons = {
        ...
        EXTargetPainter = Class(EXCEMPArrayBeam01) {
            OnWeaponFired = function(self)
				...
				--M&B: while an auto-OC shot is pending OR the retreat (FAF) mode keeps the
				--OC weapon live, freeze the weapon switch - re-enabling the gun here puts
				--the gun and the live OC weapon into a fight over the same turret
				if self.unit.MNB_OCArmed or self.unit.MNB_OCFAFMode then return end
        ...
        --M&B: on Aeon the painter switches the branch weapon INSTEAD of the gun, so while
        --a branch is active the gun never fires and its hook never runs. These branch
        --weapons carry the auto-overcharge hook themselves (artillery only fires past 25,
        --the gun covers close range, so exactly one hook is live at any moment).
        EXMiasmaArtillery01 = Class(AIFArtilleryMiasmaShellWeapon) {
            OnWeaponFired = function(self)
                AIFArtilleryMiasmaShellWeapon.OnWeaponFired(self)
                self.unit:MNB_AutoOCSafe(self:GetCurrentTarget())
            end,
        },
        ...
        EXPhasonBeam01 = Class(AeonACUPhasonLaser) {
            --M&B: stream line - auto-OC fires at the beam's own target, no search
            OnWeaponFired = function(self)
                AeonACUPhasonLaser.OnWeaponFired(self)
                self.unit:MNB_AutoOCSafe(self:GetCurrentTarget())
            end,
        },
        ...
        MNB_AutoOC = Class(ADFOverchargeWeapon) {
            ...
            OnGotTarget = function(self)
                if self:MNB_CanAutoOC() then
                    ADFOverchargeWeapon.OnGotTarget(self)
                else
...
--                                    LOG(unit.MarkLevel[8])
    ...
    WarpInEffectThread = function(self)
        ...
        WaitSeconds(2.1)
        self:SetMesh('/mods/M&B/modAcu/eal0001/EAL0001_PhaseShield_mesh', true)
    ...
    EXRegenBuffThread = function(self)
		...
		elseif self.wcBeam02 then
			self.regenammount = self.regenammount + EXBP.Enhancements.EXImprovedCoolingSystem.NewRegenRate
		elseif self.wcBeam01 then
			self.regenammount = self.regenammount + EXBP.Enhancements.EXBeamPhason.NewRegenRate
		end
		if self.wcDisruptor02 then
			self.regenammount = self.regenammount + EXBP.Enhancements.EXDisruptorrEnhancer.NewRegenRate
		elseif self.wcDisruptor01 then
			self.regenammount = self.regenammount + EXBP.Enhancements.EXDisruptorrBooster.NewRegenRate
		end
    end,
    ...
    WeaponConfigCheck = function(self)
		...
		if not self.wcBuildMode and not self.wcOCMode then
			...
			if self.wcChrono02 then
				...
				local wepFlamer02 = self:GetWeaponByLabel('EXChronoDampener02')
				wepFlamer02:ChangeMaxRadius(40)
			else
				self:SetWeaponEnabledByLabel('EXChronoDampener02', false)
			end
    ...
    --the beam line) skip the search entirely and fire at the gun's target, because
    ...
    MNB_AutoOCTry = function(self, gunTarget)
        ...
        if self.wcBeam01 or self.wcBeam02 or self.wcBeam03 then
    ...
    CreateEnhancement = function(self, enh)
		...
		elseif enh =='EXDisruptorrBooster' then
			if not Buffs['EXAeonDisruptorrHealthBoost'] then
                BuffBlueprint {
                    Name = 'EXAeonDisruptorrHealthBoost',
                    DisplayName = 'EXAeonDisruptorrHealthBoost',
                    BuffType = 'EXAeonDisruptorrHealthBoost',
                    Stacks = 'REPLACE',
                    Duration = -1,
                    Affects = {
                        MaxHealth = {
        ...
        elseif enh == 'EXShieldBattery' then
            if not Buffs['EXAeonShieldHealth1'] then
                BuffBlueprint {
                    Name = 'EXAeonShieldHealth1',
                    DisplayName = 'EXAeonShieldHealth1',
                    BuffType = 'EXAeonShieldHealth1',
                    Stacks = 'REPLACE',
                    Duration = -1,
                    Affects = {
                        MaxHealth = {
            ...
            Buff.ApplyBuff(self, 'EXAeonShieldHealth1')
        ...
        elseif enh == 'EXActiveShielding' then
            if not Buffs['EXAeonShieldHealth2'] then
                BuffBlueprint {
                    Name = 'EXAeonShieldHealth2',
                    DisplayName = 'EXAeonShieldHealth2',
                    BuffType = 'EXAeonShieldHealth2',
                    Stacks = 'REPLACE',
                    Duration = -1,
                    Affects = {
                        MaxHealth = {
            ...
            Buff.ApplyBuff(self, 'EXAeonShieldHealth2')
            self:DestroyShield()
            ForkThread(function()
                WaitTicks(1)
				self:CreatePersonalShield(bp)
        ...
        elseif enh == 'EXImprovedShieldBattery' then
            if not Buffs['EXAeonShieldHealth3'] then
                BuffBlueprint {
                    Name = 'EXAeonShieldHealth3',
                    DisplayName = 'EXAeonShieldHealth3',
                    BuffType = 'EXAeonShieldHealth3',
                    Stacks = 'REPLACE',
                    Duration = -1,
                    Affects = {
                        MaxHealth = {
            ...
            Buff.ApplyBuff(self, 'EXAeonShieldHealth3')
            self:DestroyShield()
            ForkThread(function()
                WaitTicks(1)
				self:CreatePersonalShield(bp)
        ...
        elseif enh =='EXMaelstromQuantum' then
				...
				table.insert( self.MaelstromEffects01, CreateAttachedEmitter( self, 'Torso', self:GetArmy(), '/mods/M&B/effects/emitters/exmaelstrom_aura_01_emit.bp' ):ScaleEmitter(1):OffsetEmitter(0, -2, 0) )
				table.insert( self.MaelstromEffects01, CreateAttachedEmitter( self, 'Torso', self:GetArmy(), '/mods/M&B/effects/emitters/exmaelstrom_aura_02_emit.bp' ):ScaleEmitter(1):OffsetEmitter(0, -2.75, 0) )
