...
function WrapAndPlaceText(air, physics, weapons, abilities, text, control)
	...
	local weapon_start = table.getsize(lines)
	if weapons then
	local i = table.getn(weapons)
	while weapons[i] do
		if weapons[i].WeaponCategory == nil then
		elseif weapons[i].WeaponCategory != 'Death' then
			if i > 1 and weapons[i].DisplayName == weapons[i-1].DisplayName then
				mflag = mflag + 1
			else
				if mflag != 0 then
					table.insert(lines, weapon_start + 1, string.format("%s (%s) x%d",
						weapons[i].DisplayName,
						weapons[i].WeaponCategory,
						mflag + 1))
				else
					table.insert(lines, weapon_start + 1, string.format("%s (%s)",
						weapons[i].DisplayName,
						weapons[i].WeaponCategory))
				end 
		
				if weapons[i].ProjectilesPerOnFire != nil then
					ppf = weapons[i].ProjectilesPerOnFire
				else
					ppf = 1
				end
		
				if weapons[i].DoTPulses != nil then
					dotpulses = weapons[i].DoTPulses
				else
					dotpulses = 1
				end
		
		                table.insert(lines, weapon_start + 2, string.format("Damage(%d) Rate(%0.2f) Range(%d) = DPS(%d)",
					weapons[i].Damage * ppf * dotpulses,
					1.0 / weapons[i].RateOfFire,
					weapons[i].MaxRadius,
					math.floor(weapons[i].Damage * ppf * dotpulses * weapons[i].RateOfFire)))
				mflag = 0;
			end
		end
            i = i - 1
    ...
    physics_line = table.getsize(lines) + 1
    if air != nil and air.MaxAirspeed != nil and air.MaxAirspeed !=0 then
	table.insert(lines, physics_line, string.format("MaxAirspeed(%0.2f) TurnSpeed(%0.2f)",
		air.MaxAirspeed,
		air.TurnSpeed))
    elseif physics != nil and physics.MaxSpeed != nil and physics.MaxSpeed !=0 then
	table.insert(lines, physics_line, string.format("MaxSpeed(%0.2f) MaxBrake(%0.2f) TurnRate(%d)",
		physics.MaxSpeed,
		physics.MaxBrake,
		physics.TurnRate))
    end
