...
#    	LOG('*SPAWNING FIRST SET OF DRONES')
    	...
    	### Are we IsDead yet, if not spawn drones
...
SpawnFactory = function(self)
    ...
    LOG('*spawn factory')
    --Only respawns the drones if the parent unit is not IsDead
    if not self.Dead then 
...
OnKilled = function(self, instigator, type, overkillRatio)
        ...
        if table.getn({self.BuildTable}) > 0 then
            for k, v in self.BuildTable do 
                IssueClearCommands({self.BuildTable[k]}) 
                IssueKillSelf({self.BuildTable[k]})
            end 
        end
    end,
OnStopBuild = function(self, unitBeingBuilt, order )
    SLandFactoryUnit.OnStopBuild(self, unitBeingBuilt, order)
    table.insert(self.BuildTable, unitBeingBuilt)    
end,
}
